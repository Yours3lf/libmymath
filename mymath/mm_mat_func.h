...
#define MYMATH_MATMULVEC_FUNC(t) \
  ...
  { \
    mm::impl::vec4i<t> tmp1 = vec.xxxx; \
    tmp1 *= mat[0].xyzz; \
    mm::impl::vec4i<t> tmp2 = vec.yyyy; \
    tmp2 *= mat[1].xyzz; \
...
namespace mymath
{
  ...
  template< typename t >
  inline impl::vec4i<t> determinant_helper( const impl::mat2i<t>& mat )
  {
  ...
  template< typename t >
  inline impl::vec4i<t> determinant_helper( const impl::mat3i<t>& mat )
  {
  ...
  template< typename t >
  inline impl::vec4i<t> determinant_helper( const impl::mat4i<t>& mat )
  {
