...
//angle in radians!
  template<typename ty>
  impl::quati<ty> rotate(const impl::quati<ty>& q, const ty& angle,
	  const impl::vec3i<ty>& axis)
  {
	return q * impl::quati<ty>(angle, axis);
  }

  template<typename ty>
  impl::vec3i<ty> rotate_vector(const impl::quati<ty>& q,
	  const impl::vec3i<ty>& v)
  {
