...
bool jinteresct( const vec3& rO, const vec3& rD, float& resT )
{
  float h = 0.0;
  float t = 0.0;
...
float sinteresct( const vec3& rO, const vec3& rD )
{
  float res = 1.0;
  float t = 0.0;
...
vec3 camPath( const float& time1 )
{
  vec2 p = 600.0 * vec2( cos( 1.4 + 0.37 * time1 ),
                         cos( 3.2 + 0.31 * time1 ) );
...
void main( void )
{
  ...
  vec3 campos = camPath( time1 );
  vec3 camtar = camPath( time1 + 3.0 );
  campos.y = terrain( campos.xz ) + 15.0;
  camtar.y = campos.y * 0.5;

  float roll = 0.1 * cos( 0.1 * time1 );
